...
/**
 * <p>
 * The <code>DrupalRESTConnection</code> class allows a connection to a Drupal-REST server, 
 * or any other REST server that implements the same methods.
 ...
 * <p>
 * The connection sends HTTP requests to the host under the url /=/arg1/arg2/arg3/arg4.
 * The request type (GET, POST, PUT, DELETE) determines the type of the method, 
 * where GET is a query, POST is a creation of a new item, PUT is an update method and DELETE is a deletion method.
 * <li>arg1 is mandatory and specifies the type of object (user, node, comment, buddy).</li>
 ...
 * <li>arg4 is a second member field.</li>
 * For a list of methods on the REST server connect to http://host/=/help
 * <p><br>
 * This class uses the {@link IHttpClient} interface, implemented by the {@link HttpMan} class for HTTP connection. 
 * 
 * @author yoavram
...
public class DrupalRESTConnection implements IRESTConnection {
    ...
    protected static Logger logger = Logger.getLogger(DrupalRESTConnection.class);
    ...
    /* (non-Javadoc)
	 * @see com.netalign.netascutter.IRESTConnection#setHttpClient(com.netalign.netascutter.interfaces.IHttpClient)
	 */
    @Override
    ...
    /* (non-Javadoc)
	 * @see com.netalign.netascutter.IRESTConnection#createComment(java.util.Map)
	 */
    @Override
    ...
    /* (non-Javadoc)
	 * @see com.netalign.netascutter.IRESTConnection#updateComment(java.util.Map)
	 */
    @Override
    ...
    private int updateObject(Map<String, String> map, String objectType, String key, String value) {
        int result = Constants.ILLEGAL_ID;
        String urlStr = baseUrl + objectType + "/" + key + "/" + value + JSON_POSTFIX;
        try {
            serverUrl = new URL(urlStr); // throws MalformedURLException   
    ...
    public boolean checkBuddies(int uid, int buddyUid) {
        ...
        try {
            String urlStr = baseUrl + BUDDY_RELATIVE_PATH + "/" + Integer.toString(uid) + "/" + Integer.toString(buddyUid) + JSON_POSTFIX;
    ...
    /* (non-Javadoc)
	 * @see com.netalign.netascutter.IRESTConnection#getUserId(java.lang.String, java.lang.String)
	 */
	@Override
    ...
    /* (non-Javadoc)
	 * @see com.netalign.netascutter.IRESTConnection#getNodeId(java.lang.String, java.lang.String)
	 */
	@Override
    ...
    /* (non-Javadoc)
	 * @see com.netalign.netascutter.IRESTConnection#getCommentId(java.lang.String, java.lang.String)
	 */
	@Override
    ...
    /* (non-Javadoc)
	 * @see com.netalign.netascutter.IRESTConnection#getUserFieldByKeyValue(java.lang.String, java.lang.String, java.lang.String)
	 */
	@Override
    ...
    /* (non-Javadoc)
	 * @see com.netalign.netascutter.IRESTConnection#getNodeFieldByKeyValue(java.lang.String, java.lang.String, java.lang.String)
	 */
	@Override
    ...
    /* (non-Javadoc)
	 * @see com.netalign.netascutter.IRESTConnection#getCommentFieldByKeyValue(java.lang.String, java.lang.String, java.lang.String)
	 */
	@Override
    public String getCommentFieldByKeyValue(String key, String value, String lookup) {
        return getObjectFieldByKeyValue(key, value, lookup, Constants.COMMENT);
    }
    
    private String getObjectFieldByKeyValue(String key, String value, String lookup, String objectType) {
        String result = Constants.EMPTY_STRING;
        if (key == null || key.isEmpty() || value == null || value.isEmpty() || lookup == null || lookup.isEmpty()) {
            return result;
        }
        try {
            String urlStr = baseUrl + objectType + "/" + key + "/" + URLEncoder.encode(value,Constants.UTF8_ENCODING) + "/" + lookup + JSON_POSTFIX;
            //String urlStr = baseUrl + objectType + "/" + key + "/" + value + "/" + lookup + JSON_POSTFIX;
    ...
    /* (non-Javadoc)
	 * @see com.netalign.netascutter.IRESTConnection#hasUserWithKeyValue(java.lang.String, java.lang.String)
	 */
    @Override
    ...
    /* (non-Javadoc)
	 * @see com.netalign.netascutter.IRESTConnection#hasNodeWithKeyValue(java.lang.String, java.lang.String)
	 */
    @Override
    ...
    /* (non-Javadoc)
	 * @see com.netalign.netascutter.IRESTConnection#hasCommentWithKeyValue(java.lang.String, java.lang.String)
	 */
    @Override
     ...
     * @return  the JSONObject that was read. May return an empty JSONObject if failed.
     */
    private JSONObject read(String method) {
        ...
        try {
            ...
            logger.error("Failed parsing JSON answer from " + serverUrl.toString() + ": " + e.getMessage());
        }
        return reply;
    }
     ...
     * @param key       the specific key to parse 
    ...
    private String parseJsonObject(JSONObject json, String key) {    	
        ...
        try { 
        	// parse the messages
        	if (json.has(JSON_MSGS_FIELD) && json.get(JSON_MSGS_FIELD) instanceof JSONObject) { 
    			JSONObject msgs = json.getJSONObject(JSON_MSGS_FIELD);
    			if (msgs.has(JSON_ERROR_VALUE)) {
    				logger.error(decodeJsonMessage(msgs.getString(JSON_ERROR_VALUE))); 
    				//TODO this string can be formatted to remove leading and trailing[" and other stuff
    			} else if (msgs.has(JSON_WARNING_VALUE)) {    				 
    				logger.warn(decodeJsonMessage(msgs.getString(JSON_WARNING_VALUE))); 
    				//TODO this string can be formatted to remove leading and trailing[" and other stuff
    			} else {
        	...
        	// parse value
        	if (json.has(JSON_VALUE_FIELD) && json.get(JSON_VALUE_FIELD) instanceof JSONObject) {  
        		JSONObject val = json.getJSONObject(JSON_VALUE_FIELD);
        		if (val.has(key)) {
        			output = val.getString(key);
        		} else {
        			logger.debug("Key " + key + " couldn't be found in server reply");
        		}
        	} else if (json.has(JSON_VALUE_FIELD) && json.get(JSON_VALUE_FIELD) instanceof Boolean) {
        		output = Boolean.toString( json.getBoolean(JSON_VALUE_FIELD) );        		
        	} else if (json.has(JSON_VALUE_FIELD) && json.get(JSON_VALUE_FIELD) instanceof String) {
        	...
        	if (!json.has(JSON_VALUE_FIELD) && !json.has(JSON_MSGS_FIELD)) { 
        		...
        		logger.warn("Reply from server was empty - no expected JSON objects");        		
        	}            
        } catch (JSONException e) {
            logger.warn("Failed getting key '" + key + "' from JSON object: " + e.getMessage());
        }
        return output;
    }
     ...
     * @param key the key of the field to parse
     * @return the standard java Integer.parseInt() of the JSON value by the key given
     */
    private int parseJsonObjectToInt(JSONObject json, String key) {
    	...
    	if (!str.isEmpty()) {
	    	try {
	    		...
	    		logger.debug("JSON value for key " + key + " is not numerical: " + str);
	    	}
    	}
    	return output;
    }
     ...
     * @param json	a JSON object
     * @param key the key of the field to parse
     * @return the standard java Boolean.valueOf() of the JSON value by the key given
     */
    ...
    /**
     * decodes JSON messages from the Drupal REST server, which for example, encodes " to \&quot;
    ...
    private String decodeJsonMessage(String str) {    	
    	...
    	str=str.replaceAll("&#039;", "'");
    	str=str.replaceAll("\\\\&quot;", "\"");
    	str=str.replaceAll("&amp;", "&"); //last, because it is the escape char   
    	if (str.startsWith("[\"")) {
    		start = 2;
    	} 
    	if (str.endsWith("\"]")) {
